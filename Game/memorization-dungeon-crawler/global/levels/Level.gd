...
var card_review_number:float = 1 #How many times we want to review each card in this level
var boss_card_review_number:float = 2 #How many times we want to review each card during a bossfight
...
func _init(_name: String, _theme: String, _card_review_number:float, _boss_card_review_number:float,\
		_levelType: LevelType = LevelType.STANDARD, _boss_name: String = "",\
		 _time_to_answer_sec: float = 30.0, _card_tags: Array[String] = []):
	level_name = _name
	...
	boss_card_review_number = _boss_card_review_number
...
func toString() -> String:
	...
	return "Level: \"%s\" |\t Time-Sec: %.2f |\t Theme: %s |\t Level-Type: %s |\t Boss-name: \"%s\" |\t Card-Tags: [%s] |\t Card Review #: %s; Boss: %s" % [
		level_name, time_to_answer_sec, theme_name, type_name, boss_name,  ", ".join(card_tags), card_review_number, boss_card_review_number
	]
